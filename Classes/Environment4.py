...
class Environ:
    ...
    def Compute_Performance_Reward_Train(self, actions, state_old_all, test_dataset1, test_dataset2, test_dataset3, test_dataset4):
        # 4 RSU
        ...
        # RSU4
        # 300 popular contents, 100 cache contents
        popular_content4 = []  # 推荐的缓存内容
        for i in range(self.cache_size * 2):
            popular_content4.append(state_old_all[3][i + 2 * self.cache_size])

        old_cache4 = []
        for i in range(self.cache_size * 2):
            old_cache4.append(state_old_all[3][i])

        # RSU4 更新缓存内容损耗
        replace_num4 = 0
        for i in range(len(actions[3])):
            if actions[3][i] != old_cache4[i] and actions[3][i] == 1:
                replace_num4 += 1
        replace_elements4 = replace_num4

        cache_content4 = []  # 做出action后缓存的内容
        for i in range(len(popular_content4)):  # 说明 1 是缓存, 0 是不缓存
            if actions[3][i] == 1:
                cache_content4.append(popular_content4[i])
        ...
        # RSU3内容传递
        request_number3, hit_number3, hit_number_n3 = cache_hit3(test_dataset3, cache_content3, cache_content2, cache_content4)

        # RSU4内容传递
        request_number4, hit_number4, hit_number_n4 = cache_hit(test_dataset4, cache_content4, cache_content3)

        return replace_elements1, replace_elements2, replace_elements3, replace_elements4, request_number1, \
               hit_number1, hit_number_n1, request_number2, hit_number2, hit_number_n2,\
               request_number3, hit_number3, hit_number_n3, request_number4, hit_number4, hit_number_n4


    def act_for_training(self, actions, state_old_all, test_dataset1, test_dataset2, test_dataset3, test_dataset4):
        ...
        replace_element1, replace_element2, replace_element3, replace_element4, request_number1, hit_number1, hit_number_n1,\
        request_number2, hit_number2, hit_number_n2, request_number3, hit_number3, hit_number_n3, request_number4, hit_number4, hit_number_n4\
            = self.Compute_Performance_Reward_Train(action_temp, state_old_all, test_dataset1, test_dataset2, test_dataset3, test_dataset4)
        ...
        per_user_reward[3] = (self.c_cost - self.l_cost) * hit_number4 + (self.c_cost - self.n_cost) * hit_number_n4 \
                             - self.r_cost * replace_element4
        ...
        print('replace_element1 : ', replace_element1, 'user1_reward : ', per_user_reward[0],
              ...
              'replace_element4 : ', replace_element4, 'user4_reward : ', per_user_reward[3],)
        ...
        per_user_hit_radio[3] = (hit_number4 + hit_number_n4) / request_number4 * 100
        per_user_cost[3] = self.l_cost * hit_number4 + self.n_cost * hit_number_n4 + self.c_cost * (
                request_number4 - hit_number4 - hit_number_n4) + \
                           self.r_cost * replace_element4
        #per_user_cost[1] = self.l_cost * hit_number2 + self.n_cost * hit_number_n2 + self.c_cost * (request_number2 - hit_number2 - hit_number_n2)
