...
import Classes.Environment4 as ENV
...
# RSU4
# -----
sample4, users_group_train4, users_group_test4, _ = sampling(args, n_veh)
data_set4 = np.array(sample4)
# test_dataset & test_dataset_idx
test_dataset_idxs4 = []
for idx in range(n_veh):
    test_dataset_idxs4.append(users_group_test4[idx])
test_dataset_idxs4 = list(chain.from_iterable(test_dataset_idxs4))
test_dataset4 = data_set3[test_dataset_idxs4]
...
# RSU3 position
s2 = np.random.randint(3000,3300,n_veh)
...
for i in range(int(n_veh*n_rsu)):
    ...
    if n_veh * 3 <= i < n_veh * 4:
        veh_pos.append(s2[i - 30])
...
in_si4 = int(max(data_set4[:, 1]))
clo4 = int(max(data_set4[1, :]))

netE4 = Encoder(input_size=in_si4, hidden_size=[256, 64, 16, 4, 2])
netE4.to(device)
netP4 = Decoder(output_size=in_si4, hidden_size=[2, 4, 16, 64, 256])
netP4.to(device)
netD4 = Discriminator(input_size=2, hidden_size=[256, 16, 4, 2], output_size=1)
netD4.to(device)
print('initial RSU2 global model:', netE4, netP4, netD4)
...
wwh_test_w_e_all_epochs4 = env.wwh_test_get_lr_para(netE4, netP4, netD4, data_set4, users_group_train4)
...
if IS_TRAIN:
    ...
    for i_episode in range(args.slots):
        ...
        recommend_movies_cs4, netE4, netP4, netD4 = env.get_content_pop(netE4, netP4, netD4, data_set4, sample4,
                                                                       users_group_train4, users_group_test4)
        ...
        # rsu 4
        state4 = get_state(env, recommend_movies_cs4)
        state_old_all.append(state4)
        ...
        for i_step in range(n_step_per_episode):
            ...
            train_reward, global_reward, train_hit_radio, train_cost = \
                env.act_for_training(action_temp, state_old_all, test_dataset1, test_dataset2, test_dataset3, test_dataset4)
            ...
            state_new4 = get_new_state(action_temp[3], recommend_movies_cs4)
            state_new_all.append(state_new4)
        ...
        if plot_i % 100 == 0:
            ...
            plt.plot(x,record_global_reward_[0][:i_episode],
                     ...
                     x,record_reward_[3][:i_episode],)
            plt.legend(["global reward","RSU1 reward","RSU2 reward","RSU3 reward","RSU4 reward"])
            plt.show()
    ...
    netE4.save_checkpoint()
    netP4.save_checkpoint()
    netD4.save_checkpoint()
    print("over")
    # global_agent.save_models()
    # for i in range(n_rsu):
    #     agents[i].save_models()